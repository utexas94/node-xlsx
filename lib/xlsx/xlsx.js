...
function xlsx(file) { 'use strict'; // v2.2.0
	var result, zip = new JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, style,
		...
		numFmts = ['General', '0', '0.00', '#,##0', '#,##0.00', '"$"#,##0',,,, '0%', '0.00%', '0.00E+00', '# ?/?', '# ??/??', 'mm-dd-yy', 'd-mmm-yy', 'd-mmm', 'mmm-yy', 'h:mm AM/PM', 'h:mm:ss AM/PM',
		...
		//{ Content dependent
			...
			while (w--) { // Generate worksheet (gather sharedStrings), and possibly table files, then generate entries for constant files below
				...
				//{ Generate sheetX.xml in var s
                    ...
                    var columnWidths = worksheet.columnWidths;
                    if (columnWidths) {
                      s += '<cols>';
                      i = -1; l = worksheet.columnWidths.length;
                      while (++i < l) {
                        s += '<col customWidth="1" width="' + columnWidths[i] + '" max="' + (i+1) + '" min="' + (i+1) + '"/>';
                      }
                      s += '</cols>';
                    }
				...
				worksheets.unshift('<sheet name="' + ((escapeXML(worksheet.name) && (escapeXML(worksheet.name)).substring(0,31)) ||  'Sheet' + id) + '" sheetId="' + id + '" r:id="rId' + id + '"/>');
			}
